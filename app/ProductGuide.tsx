...
const guides = {
  methodology: {
    ...
    sections: [
      ...
      ["Check the record", "Your browser reads the pinned registry on chain 1952, hashes the corresponding original document and recomputes all holding values. A match means the document, calculation and recorded NAV agree. Missing evidence remains unverified. The verification page also edits a browser copy three ways to show which check catches each change, and offers an evidence file that npm run verify:evidence re-checks against the publishing transaction's NavPublished event."],
      ["Record the shares outstanding", "Each NAV record also carries the USTX shares outstanding in the demo ledger when the NAV was taken, so the fund size shown in the app is the recorded shares multiplied by the recorded NAV. A retried publication sends the count stored with it, never a newer one."],
  ...
  limitations: {
    ...
    sections: [
      ["Demo investing, testnet record", "Investing on Ganymede uses demo dollars. No real money moves, Ganymede does not hold or custody the xStocks, and USTX shares exist only in the demo ledger, not on chain. Constituent prices come from X Layer mainnet (196); NAV records are published to X Layer Testnet (1952). Smart contracts are unaudited. Testnet records do not establish legal rights or asset backing."],
      ...
      ["Price eligibility and delayed updates", "OnchainOS stamps each quote with the time of the response, and a quote more than 10 minutes old (the default, configurable by the operator) or a missing constituent price blocks a new NAV; USTX does not substitute reference prices. A record’s time is when the NAV was calculated, so every price in it is at most 10 minutes older than that time; the USTX fund and the lending market accept a record for one hour after it. The displayed cycle-delay warning starts after 15 minutes without a recent pricing attempt. These are different checks, and neither guarantees quote accuracy. Transparency shows the configured quote-age limit with its calculation details."],
