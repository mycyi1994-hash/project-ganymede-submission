...
- **Technical execution.** Integer arithmetic, canonical document bytes, a pinned registry and network, direct RPC reads, receipt-event matching, transactional demo orders with idempotent retries, failure tests (162 application tests, 28 relayer tests, 62 contract tests) and isolated visitor records. Wallet reads are pinned to the newest block seen, and every order is dry-run before the wallet opens. A quote more than ten minutes old blocks a NAV, so a record is never more than ten minutes newer than its prices.
...
- **What the check covers.** Prices come from one source, so a publisher that records wrong prices in a consistent document passes every check; a second, independent price source is the next step. Record consistency is not asset backing: the testnet fund holds no xStocks, burning an investment's demo dollars and minting a redemption's. Contracts are unaudited.
