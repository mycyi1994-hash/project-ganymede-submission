...
export default function BasketOverview() {
  ...
  const total = composition?.holdings.reduce((sum, holding) => sum + BigInt(holding.valueMicros), 0n);
  const active = composition?.holdings.find(holding => holding.symbol === selected);
  return <section className="basket-overview" id="ustx-basket" aria-labelledby="ustx-basket-title">
    <header><div><h2 id="ustx-basket-title">What makes up one model share?</h2><p>Select a holding to inspect its contribution. Prices come from OKX OnchainOS on X Layer mainnet.</p></div></header>
    ...
    <div className="proof-table-wrap"><table className="proof-table"><thead><tr><th scope="col">Token</th><th scope="col">Weight at fixing</th><th scope="col">Token price / USD</th><th scope="col">Value per model share</th></tr></thead><tbody>{XSTOCKS_CONSTITUENTS.map(item => {
      ...
      return <tr key={item.symbol} className={selected === item.symbol ? "holding-selected" : undefined}><th scope="row"><button className="basket-token holding-select" type="button" aria-pressed={selected === item.symbol} onClick={() => setSelected(item.symbol)}><StockMark symbol={item.symbol} /><span>{item.symbol}<small>{item.name}</small></span></button></th><td>{holding ? `${(holding.weightBps / 100).toFixed(2)}%` : "—"}</td><td>{holding ? formatUsdMicros(holding.priceMicros, 4) : "—"}</td><td>{holding ? formatUsdMicros(holding.valueMicros, 4) : "—"}</td></tr>;
    })}</tbody></table></div>
    ...
    <div className="basket-contribution" role="status"><div><span>{selected} contribution per model share</span><strong key={selected}>{active ? formatUsdMicros(active.valueMicros, 4) : "—"}</strong></div><div><span>Sum of all six holdings</span><strong>{total !== undefined ? formatUsdMicros(total.toString(), 4) : "—"}</strong></div><p>These values belong to the pricing snapshot above. The published report may use an earlier snapshot.</p></div>
    <footer><div><h3>Does the published report match?</h3><p>Compare the published calculation and document with the X Layer record.</p></div><a className="button is-primary" href="/proof">Verify this basket</a></footer>
  </section>;
}
