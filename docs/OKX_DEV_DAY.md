...
- Another issuer's basket needs a configuration file, not changes to our code: MAG3, a three-stock demo basket, is recorded by a demo issuer wallet we created, separate from our keys, in its own registry on X Layer Testnet, priced from the X Layer pools alone, and the developer page and `/embed/basket` verify it with USTX's record checks plus a check of its configured units, weights and fixing time.
...
- Source verification: all eight X Layer Testnet contracts are verified on the OKX explorer and match exactly on Sourcify, as does MAG3's registry deployed by the demo issuer wallet, for example USTX at https://repo.sourcify.dev/1952/0x77eaeba1366bde7818da12d3cbdbea0a2ee97596
...
Ganymede began in July 2026 as a Korean-won crypto strategy engine with Upbit market data and settlement built for the GIWA Sepolia testnet. The last commit before the event was `7a33392` on 30 July. Reused from that codebase and not counted as new work: the strategy engine (its strategies run the separate paper Lab; its D1 state store and lease also serve the five-minute USTX step added in the build period), the fixed-point helpers, the NAV registry and share-ledger contracts (redeployed to X Layer Testnet without changes) and the settlement relayer (extended for X Layer). Every commit of the build period, with times and line counts, is in [BUILD_PERIOD.md](BUILD_PERIOD.md). The main pieces:
...
- **Innovation.** On-chain NAV exists elsewhere, for example in the DTCC Smart NAV pilot and in Centrifuge. The contribution here is narrower: any visitor reproduces a basket NAV row by row against an X Layer record. USTX also trades on its own X Layer pool, and one contract closes the pool's gap to the NAV through the fund, the ETF creation and redemption mechanism in a single transaction, which a keeper checks for every five minutes and runs when it earns at least a cent. The experiment isolates what the chain fingerprint adds, and the result travels as a verifiable file. The recorded prices are compared with a second source, the X Layer pools, before each record and again in the browser.
...
- **User value.** An investor buys a diversified tech basket in three taps, sees the exact tokens behind each share, and can confirm in seconds that the price they paid matches its document and chain record, then pass that confirmation on as a file instead of a screenshot. Against buying the six xStocks separately, one USTX order takes two wallet confirmations instead of seven or more, leaves one position instead of six and rebalances inside the basket; network fees are a fraction of a cent on either path (the README compares the two with measured gas).
- **Technical execution.** Integer arithmetic, canonical document bytes, a pinned registry and network, direct RPC reads, receipt-event matching, transactional demo orders with idempotent retries, failure tests (180 application tests, 28 relayer tests, 69 contract tests) and isolated visitor records. Wallet reads are pinned to the newest block seen, and every order is dry-run before the wallet opens. A record carries the time of its oldest price, a quote more than ten minutes old blocks it, the fund and the lending market count their one-hour limit from that time, and prices quoted more than a minute apart block it, and the browser and the evidence command reject a price more than a minute older than the record time and a record time more than a minute after its block.
...
- **Growth and ecosystem.** Other X Layer apps can show the verified NAV through the public API or the self-verifying badge, and contracts can read it on chain through the Chainlink-style NAV feed. The README sets out the business model (free sandbox, per-basket subscription, distribution fee through licensed partners), its measured unit cost (a basket recorded every five minutes uses about $1.40 of X Layer gas a month at 25 September's gas price) and the next steps to X Layer mainnet and an issuer console; the issuer page offers the plans. A second basket, MAG3, already runs from one configuration file in a demo issuer's own registry with its own badge, and an in-kind vault for real xStocks has run on a mainnet fork. USTX is also collateral: a lending market on X Layer Testnet lends demo dollars against it at the recorded NAV, and the USTX page borrows, repays and lends through it from OKX Wallet. No adoption is claimed.
...
- **What the check covers.** The consistency checks cannot catch a wrong input, so the OnchainOS prices are compared with a second source, the X Layer mainnet Uniswap V3 pools of the xStocks' wrappers: the publisher does not record a NAV more than 1% from its value at the pool prices (if the pools cannot be read, the record goes ahead with a warning), and the Transparency page repeats the comparison in the browser. Both are X Layer markets, so a price wrong in both passes, and neither is compared with the stock exchange. Record consistency is not asset backing: the testnet fund holds no xStocks, burning an investment's demo dollars and minting a redemption's. An in-kind vault that would hold them, `GanymedeBasketVault`, ran with the real xStocks on a fork of X Layer mainnet (`npm run fork:vault`, docs/IN_KIND_VAULT.md) and is not deployed. Contracts are unaudited.
