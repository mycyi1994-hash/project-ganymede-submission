...
export default function Holdings({ composition, compact = false, loading = false }: { composition: Composition | null; compact?: boolean; loading?: boolean }) {
  ...
  return <section className={`gmd-holdings ${compact ? "is-compact" : ""}`} aria-label="Basket composition"><header className="gmd-section-heading"><div><h2>{compact ? "The basket" : "Constituents"}</h2><p>{composition ? "Weights at the latest OKX OnchainOS prices" : "Six US technology xStocks"}</p></div><span className="gmd-count">6 assets</span></header>
    ...
    <div className="gmd-holdings-table"><table className="gmd-table"><thead><tr><th>Asset</th><th>Weight</th>{!compact && <><th>OKX price</th><th>Value / share</th></>}</tr></thead><tbody>{assetSymbols.map(symbol => {
    ...
    <div className="gmd-contribution" aria-live="polite"><span><b>{assetNames[selected]}</b>{active ? " contributes per share" : loading ? " · loading holdings…" : " · holdings unavailable"}</span><strong key={selected}>{active ? formatUsdMicros(active.valueMicros, 4) : "—"}</strong></div>
    {!compact && <p className="gmd-caption">Equal weight at each quarterly rebalance; weights move with prices. {composition ? `Prices from OKX OnchainOS as of ${shortTime(composition.asOf)}.` : "Values appear with the next recorded NAV."}</p>}
  </section>;
}
