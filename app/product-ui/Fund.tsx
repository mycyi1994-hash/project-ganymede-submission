...
export function FundOverview() {
  ...
  return <section id="overview" className="gmd-fund" aria-labelledby="fund-title">
    ...
    <div className="gmd-fund-grid" aria-busy={figures.pending}>
      <article><span>Fund size</span><strong>{figures.size === null ? figures.pending ? <Skeleton width={96} /> : "—" : formatUsdRounded(figures.size)}</strong><small>{figures.shares === null ? figures.pending ? <Skeleton width="80%" /> : "Unavailable" : figures.onChain && record ? `${formatSharesShort(figures.shares)} shares outstanding, recorded on X Layer at ${shortTime(record.effectiveAt)}` : `${formatSharesShort(figures.shares)} shares outstanding; recorded on X Layer with the next NAV`}</small></article>
