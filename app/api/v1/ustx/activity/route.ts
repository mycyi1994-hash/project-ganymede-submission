...
import { activityJson, parseActivityIndex } from "@/lib/xstocks/activity";
...
export async function GET() {
  try {
    ...
    return json({
      ...
      rule: "Every event of these contracts from fromBlock to toBlock, newest first, at most 40 rows. An arbitrage is one row: its pool trade and fund order are folded in. Amounts are micros (6 decimals).",
      rows: index.rows.map(row => ({ ...activityJson(row), explorerUrl: fundExplorer.tx(row.hash) })),
