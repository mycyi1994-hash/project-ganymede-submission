...
export default function MarketChart({ points, total = points.length, loading }: { points: HistoryPoint[]; total?: number; loading: boolean }) {
  ...
  return <div className="gmd-chart-block"><div className="gmd-chart-heading"><span>{active ? `${shortTime(active.at)} · ${formatUsdMicros(active.micros, 4)}` : "Published NAV history"}</span><div className="gmd-segmented" aria-label="History display"><button aria-pressed={view === "chart"} onClick={() => setView("chart")}>Chart</button><button aria-pressed={view === "records"} onClick={() => setView("records")}>Records</button></div></div>
    {view === "records" ? <div className="gmd-records-scroll"><table className="gmd-table"><caption className="gmd-sr-only">Available published NAV records</caption><thead><tr><th>Effective at</th><th>NAV / USD</th></tr></thead><tbody>{[...points].reverse().map(point => <tr key={point.at}><td>{shortTime(point.at)}</td><td>{formatUsdMicros(point.micros, 4)}</td></tr>)}</tbody></table>{!points.length && <p className="gmd-muted">{loading ? "Loading publication history…" : "No confirmed history is available."}</p>}</div> : points.length < 2 ? <div className="gmd-chart-empty"><div className="gmd-chart-empty-grid" aria-hidden="true" /><p>{loading ? "Loading recorded values…" : points.length === 1 ? "One published record. More history will appear as values are published." : "Publication history is not available."}</p></div> : <>
