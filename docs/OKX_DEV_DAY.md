...
- **Technical execution.** Integer arithmetic, canonical document bytes, a pinned registry and network, direct RPC reads, receipt-event matching, transactional demo orders with idempotent retries, failure tests (140 application tests, 28 relayer tests, 61 contract tests) and isolated visitor records. Wallet reads are pinned to the newest block seen, and every order is dry-run before the wallet opens.
